...
import { Check, ImageIcon, Minus, Plus, ShoppingCart, X } from "lucide-react";
...
export function MenuItemDetailsSheet({
  ...
  // Removing an ingredient is a preparation instruction, not a discount —
  // it never changes the price (mirrors the server's pricing rule).
  ...
  const toggleIngredient = (ingredientName: string) => {
    setRemovedIngredients((current) => {
      if (current.includes(ingredientName)) {
        return current.filter((name) => name !== ingredientName);
      }
                ...
                {item.ingredients.length > 0 && (
                  <section>
                    ...
                    <ul
                      ...
                      aria-label="Ingredients — tap to remove"
                    >

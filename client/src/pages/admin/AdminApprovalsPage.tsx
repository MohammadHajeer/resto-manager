 ...
 * -----------------------
 * RES-35: Create pending restaurants page
 *
 * PURPOSE:
 *   Shows the admin a list of all restaurant registrations that are
 *   currently PENDING approval. Each row lets the admin click through
 *   to the full review page (AdminRestaurantReviewPage) where they
 *   can approve or reject.
 *
 * ROUTE:
 *   /admin/approvals  (inside AdminLayout — already wired in App.tsx)
 *
 * DATA:
 *   Currently uses LOCAL mock data so the UI is fully functional
 *   and testable without a backend.
 *   TODO: Replace MOCK_PENDING with a real API call:
 ...
 *   - "Review" button → navigates to /admin/approvals/:restaurantId
 *
 * EMPTY STATE:
 *   If there are no pending registrations, a friendly message is shown
 *   instead of an empty table.
 *
 * DESIGN MATCH:
 *   Matches the AdminLayout visual style:
 *   - White card on #F8FAF9 background
 *   - #0F172A text, #6B7280 muted text
 *   - #16A34A green for the Review CTA
 *   - Consistent border colour: #E2E8F0
 */

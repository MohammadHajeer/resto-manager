...
import type { OwnerDashboardPeriod, OwnerDashboardResponse } from "@/services/owner/owner.types";

import { formatChartDate } from "../../../components/owner/dashboard/dashboard-utils";
...
type OwnerOrdersChartProps = {
  data: OwnerDashboardResponse["ordersByDate"];
  period: OwnerDashboardPeriod;
  onPeriodChange: (period: OwnerDashboardPeriod) => void;
};

export function OwnerOrdersChart({ data, period, onPeriodChange }: OwnerOrdersChartProps) {
  return (
    <Card className="min-w-0">
      <CardHeader className="grid-cols-[1fr_auto]">
        <div>
          <CardTitle>Orders over time</CardTitle>
          <CardDescription>Daily order count, including all statuses.</CardDescription>
        </div>
        <Select
          value={String(period)}
          onValueChange={(value) => onPeriodChange(Number(value) as OwnerDashboardPeriod)}
        >
